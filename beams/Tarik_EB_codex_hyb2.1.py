...
train_df = pd.read_excel("train_data_for PINN_disp_TS_v5.xlsx", skiprows=1, usecols=[0], header=None)
anchors = train_df.values.astype(np.float32)


# here = Path(__file__).resolve().parent if "__file__" in globals() else Path.cwd()
# data_path = here / "train_data_for PINN_disp_TS_v5.xlsx"
# train_df = pd.read_excel(data_path, skiprows=1, usecols=[0], header=None)
# anchors = train_df.values.astype(np.float32)
...
# derive domain limits from data
x_left = float(anchors.min())
x_right = float(anchors.max())
...
L = x_right  # keep L for use in the analytical solution if needed
...
geom = dde.geometry.Interval(x_left, x_right)
...
def boundary_l(x, on_boundary):
    return on_boundary and np.isclose(x[0], x_left)  #isclose() function, which checks whether two values are numerically close — accounting for small floating-point errors.


# Function to identify the right (free) boundary x = L
def boundary_r(x, on_boundary):
    return on_boundary and np.isclose(x[0], x_right) 
...
data = dde.data.PDE(   # creates the dataset for solving PDE
    ...
    num_boundary=0,         # randomly pick points near x=0 and x=L to enforce BCs
